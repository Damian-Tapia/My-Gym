...
export function NavBar({
  ...
  style,
  ...rest
}: NavBarProps & Omit<React.HTMLAttributes<HTMLElement>, keyof NavBarProps>) {
  return (
    <header
      {...rest}
      style={{
        display: 'flex', flexDirection: large ? 'column' : 'row', alignItems: large ? 'flex-start' : 'center',
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
        ...
        {!large
          ? <span style={{ flex: 1, minWidth: 0, font: 'var(--text-h3)', color: 'var(--text-primary)' }}>{title}</span>
          : <span style={{ flex: 1 }} />}
        {action}
      </div>
      {large ? (
