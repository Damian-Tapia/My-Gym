...
export function RestTimer({
  ...
  style,
  ...rest
}: RestTimerProps & Omit<React.HTMLAttributes<HTMLDivElement>, keyof RestTimerProps>) {
  ...
  return (
    <div
      {...rest}
      style={{
        display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', background: 'var(--surface-raised)',
    ...
    >
      ...
      {onSkip ? (
        <button type="button" onClick={onSkip} style={{ background: 'transparent', border: 'none', cursor: 'pointer', font: 'var(--text-caption)', color: 'var(--text-muted)' }}>Saltar</button>
      ) : null}
    </div>
  );
}
