...
export function StatTile({ label, value, unit, icon, delta, deltaTone = 'success', style, ...rest }) {
  const dc = deltaTone === 'danger' ? 'var(--status-danger-fg)' : 'var(--status-success-fg)';
  return (
    <div {...rest} style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: 'var(--card-padding)',
      ...
      {delta ? <span style={{ display: 'flex', alignItems: 'center', gap: 4, font: 'var(--text-caption)', color: dc }}>
        <Icon name="trending-up" size={12} />{delta}</span> : null}
    </div>
  );
}
