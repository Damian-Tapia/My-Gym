...
export function SetRow({ index, weight, reps, unit = 'kg', done = false, active = false, onToggle, style, ...rest }) {
  return (
    <div onClick={onToggle} {...rest} style={{ display: 'grid', gridTemplateColumns: '28px 1fr 1fr 32px', alignItems: 'center', gap: 12,
      ...
      <span style={{ font: 'var(--text-metric)', fontSize: 'var(--fs-body-lg)', color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>
        {weight}<span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 3 }}>{unit}</span></span>
      <span style={{ font: 'var(--text-metric)', fontSize: 'var(--fs-body-lg)', color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>
        {reps}<span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 3 }}>reps</span></span>
